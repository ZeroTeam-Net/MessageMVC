...
function onAjaxComplete(title, jsonStr) {
    try {
...
CardEditor.prototype.reLoadData = function () {
    ...
    if (this.isAddNew || !this.dataId)
        this.dataId = 0;
    if (this.dataUrl) {
        var url = this.dataUrl + this.dataId;
        if (this.formArg)
            url += '&' + this.formArg;
        doOperator("编辑", url, null, function (result) {
            me.onLoaded(result);
        });
    } else if (this.dataId === 0 || this.data) {
        if (!this.data)
            this.data = { Id: 0 };
...
CardEditor.prototype.save = function () {
    ...
    if (!this.beforeSave())
        return;
    var me = this;
    call_ajax("保存", this.saveUrl + this.dataId, getFormJson("#" + this.form.attr('id')), function (r) {
        me.onSaved(r);
    });
...
CardEditor.prototype.onSaved = function (r) {
    var result = onAjaxComplete("保存", r);
...
CardEditor.prototype.doValidate = function (id) {
    ...
    doSilentOperator("数据校验", this.validatePath + "validate", { selects: id }, function (result) {
        if (result.succeed) {
            return;
        }
        try {
...
function doSave(title, formid, url, onsubimt, onSucceed) {
    ...
    if (!$(formid).form("validate"))
        return;
    call_ajax(title, url, getFormJson(formid), function (result) {
        result = onAjaxComplete(title, result);
        if (result && onSucceed)
            onSucceed(result);
    });
}
function submitData(formid, url, onSucceed) {
    if (!$(formid).form("validate"))
        return;
    call_ajax("提交", url, getFormJson(formid), function (result) {
        result = onAjaxComplete("提交", result);
        if (result && onSucceed)
            onSucceed(result);
    });
}
...
function doDelete(id, url, onSucceed) {
    doOperator("删除", url, { ID: id }, function (result) {
        if (onSucceed)
            onSucceed(result);
        else
            reload();
 ...
 * @param {string} txtField 文本字段
 */
function setComboData(eid, datas, callback, tree, txtField) {
    try {
        if (!txtField)
            txtField = "text";
        var data = [];
        for (var idx = 0; idx < datas.length; idx++) {
            data.push({
                id: datas[idx].id,//Number(),
                text: datas[idx][txtField]
            });
        }
        var vl;
        var opt = { valueField: "id", textField: "text", data: data };
        if (tree) {
