...
GridPanel.prototype.createGrid = function (options) {
    ...
    options.onBeforeLoad = function (arg) {
        ...
        call_ajax("载入", me.listUrl, arg, function (result) {
...
GridPanel.prototype.getMulitArgs = function (check, arg) {
    ...
    arg.selects = trimStr(arg.selects, ",");
    return true;
};
/**
 * 执行多选操作
 ...
 * @returns {void} 
 */
GridPanel.prototype.doSelectRemote = function (title, api, arg, callBack) {
    this.doRemote(title, api, null, callBack, null, arg);
};
/**
 * 执行多选远程操作
 * @public 
...
GridPanel.prototype.createToolbarButton = function (eid, title, action, icon, condition, isSelectEvent, eventFunc, noPowerFunc, reg) {
    ...
    try {
        ...
        if (!condition) {
            console.log(`${eid} : no role supper`);
            $(reg).hide();
            return;
        }
