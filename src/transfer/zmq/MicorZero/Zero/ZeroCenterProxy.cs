...
namespace ZeroTeam.ZeroMQ.ZeroRPC.ZeroManagemant
{
    ...
    internal class ZeroCenterProxy : HeartManager
    {
        ...
        {
            return HeartLeft("SystemManage", ZeroAppOption.Instance.TraceName);
        }
        ...
        {
            return HeartReady("SystemManage", ZeroAppOption.Instance.TraceName);
        }
        ...
        {
            return HeartJoin("SystemManage", ZeroAppOption.Instance.TraceName);
        }
        ...
        {
            return Heartbeat("SystemManage", ZeroAppOption.Instance.TraceName);
        }
        ...
        {
            if (ZeroAppOption.Instance.TryGetConfig(station, out _))
                return true;
        ...
        public bool UploadDocument()
        {
            if (_documentIsUpload)
                return true;
            bool success = true;
            foreach (var doc in ZeroAppOption.Instance.Documents.Values)
            {
                if (!doc.IsLocal)
                    continue;
        ...
        public bool LoadAllConfig()
        {
            ...
            return ZeroAppOption.Instance.FlushConfigs(ZeroAppOption.Instance.Master, json);
        }
        ...
        public StationConfig LoadConfig(string stationName)
        {
            ...
            if (!result.TryGetString(ZeroFrameType.Status, out var json) && json[0] == '{')
                return !ZeroAppOption.Instance.UpdateConfig(ZeroAppOption.Instance.Master, stationName, json,out var config)
                    ? null
                    : config;
