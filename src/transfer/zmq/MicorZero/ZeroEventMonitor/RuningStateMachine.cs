...
namespace ZeroTeam.ZeroMQ.ZeroRPC.ZeroManagemant
{
    internal partial class MonitorStateMachine
    {
        ...
        /// 监控状态机
        /// </summary>
        private class RuningStateMachine : IMonitorStateMachine
        {
            /// <summary>
            ///     收到信息的处理
            /// </summary>
            async Task IMonitorStateMachine.OnMessagePush(ZeroNetEventType zeroNetEvent, string station, string content)
            {
                switch (zeroNetEvent)
                {
                    ...
                    case ZeroNetEventType.CenterSystemClosing:
                        ...
                        await center_closing(ZeroRpcFlow.Config.StationOption.ServiceName);
                        return;
                    case ZeroNetEventType.CenterSystemStop:
                        StateMachine = new EmptyStateMachine();
                        await center_stop(ZeroRpcFlow.Config.StationOption.ServiceName);
                        return;
