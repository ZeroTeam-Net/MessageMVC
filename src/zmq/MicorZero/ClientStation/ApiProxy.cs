...
namespace Agebull.MicroZero.ZeroApis
{
    ...
    ///     API客户端代理
    /// </summary>
    public class ApiProxy : IAppMiddleware
    {
        #region Socket
        ...
        public Task Start()
        {
            ...
            task.ContinueWith(task => OnLoopComplete());
            return task;
        }
        ...
        public void OnLoopBegin()
        {
            ...
            foreach (var config in MicroZeroApplication.Config.GetConfigs())
            {
        ...
        public void Close()
        {
            MicroZeroApplication.ZeroNetEvents.Remove(OnZeroNetEvent);
        ...
        private long CreateSocket(ProxyCaller2 caller, out ZSocketEx socket)
        {
            ...
            if (!MicroZeroApplication.Config.TryGetConfig(caller.Station, out caller.Config))
            {
                caller.Result = ApiResultIoc.NoFindJson;
                caller.State = ZeroOperatorStateType.NotFind;
                socket = null;
